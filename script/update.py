...
"""
Connect to salesforce via API and then Simple_Salesforce
"""


sfdcUrl = "https://login.salesforce.com/services/oauth2/token"
params = {
    "grant_type": "password",
...
sf = Salesforce(instance_url=instance_url,
...
def insertTimestamp():
    ...
    _pointa = Point("log").tag("type", "datetime").field(
        "last update", datetime.now().strftime("%m/%d/%Y, %H:%M:%S"))
...
def getsalesforcelicense():
    ...
    result2 = sf.query_all(
        "select Id, Name, UsedLicenses, TotalLicenses, Status, MasterLabel, LicenseDefinitionKey from UserLicense WHERE Name ='Salesforce'")
    remains = result2['records'][0]['TotalLicenses'] - \
        result2['records'][0]['UsedLicenses']
    if (result2['records'][0]['TotalLicenses'] != 0):
        percent_used = (result2['records'][0]['UsedLicenses'] /
                        result2['records'][0]['TotalLicenses']) * 100
...
def getSalesforceApexlog():
    ...
    result = json.dumps(sf_api_call(
        '/services/data/v53.0/tooling/query/?q=SELECT+Request,+LogUserId,+Status,+DurationMilliseconds,+SystemModstamp+from+ApexLog'), indent=2)
...
def getsalesforceInstance():
    ...
    responsestatus = requests.get(
        "https://api.status.salesforce.com/v1/instances/" + pod + "/status/preview?childProducts=false")
