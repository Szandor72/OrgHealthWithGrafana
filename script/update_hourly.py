...
def login_log():
    result = sf.query_all("SELECT id, ApiType, ApiVersion, Application, AuthenticationServiceId, AuthMethodReference, Browser, ClientVersion, CountryIso,  \
                            LoginGeo.City, LoginGeo.Longitude, LoginGeo.Latitude, SourceIp, Status, LoginTime, Platform, UserId  \
                            FROM LoginHistory \
...
def get_SecurityHealthCheck():
    score = sf_api_call(
        '/services/data/v53.0/tooling/query/?q=SELECT+Score+FROM+SecurityHealthCheck')
...
def get_SecurityHealthCheckRisks():
    ...
    response = sf_api_call(
        '/services/data/v53.0/tooling/query/?q=SELECT+RiskType,+Setting,+SettingGroup,+OrgValue,+StandardValue+FROM+SecurityHealthCheckRisks+where+RiskType=\'HIGH_RISK\'+OR+RiskType=\'MEDIUM_RISK\'')
